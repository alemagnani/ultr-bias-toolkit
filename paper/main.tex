...
The results (Table~\ref{tab:yahoo_results}) confirm that harmonic weighting reduces variance by \textbf{39.5\%} versus original at ratio $r=50$, consistent with the fully synthetic results in Experiment~1 ($\sim$44\%). The relative ordering of all schemes is identical to the fully synthetic setting, confirming that variance reduction is driven by impression imbalance and not by the choice of relevance distribution.
...
Despite advances, propensity estimation remains prone to high variance, especially when utilizing importance sampling techniques \cite{swaminathan2015CRM}. Researchers have introduced numerous variance-reduction strategies such as clipping \cite{bottou2013counterfactual}, self-normalized estimators \cite{swaminathan2015batch}, doubly robust estimators \cite{dudik2011doubly, oosterhuis2023DoublyRobustLTR}, and control variates \cite{kong1992note}. Notably, doubly robust methods specifically tailored for learning-to-rank have shown substantial variance reduction and efficiency improvements \cite{oosterhuis2023DoublyRobustLTR}. These methods target the LTR ranking objective (policy gradient over a relevance model), and are not directly comparable to intervention harvesting propensity estimation, which is model-free. Within the intervention harvesting family, applying SNIPS-style normalization (dividing by $\sum_i \omega_i$) to both $A$ and $B$ is a no-op: normalizing both by the same scalar leaves $\hat{R} = A/B$ unchanged. SNIPS addresses absolute-quantity estimation; our setting is ratio estimation. Clipped weighting \cite{bottou2013counterfactual} is the natural biased low-variance reference, which we include in our comparison.
